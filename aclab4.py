...
def parameter_sweep(w_array, cl, file_path, xfoil_path):
    ...
    for i in range(len(w_array)):
        cd[i] = (run_xfoil_wcl(w_array[i], cl, file_path, xfoil_path))
    return cd
...
def one_dim_opt(x0, cl, file_path, xfoil_path):
    opt_out = minimize_scalar(run_xfoil_wcl,\
              args = (cl, file_path, xfoil_path),\
              method = 'brent', bracket=[0.6, 0.9, 1.2])
    return opt_out
    

def four_dim_opt(x0, weight_limits, cl, file_path, xfoil_path):
    opt_out = fmin_l_bfgs_b(run_xfoil_wcl, x0,\
    args = (cl, file_path, xfoil_path), bounds = weight_limits,\
    epsilon = 0.0001, approx_grad = True)
    return opt_out